...
A remote backend stores the Terraform state file outside of your local file system and version control. Using Cloud storage(S3/Blob) as a remote backend is a popular choice due to its reliability and scalability. Here's how to set it up:
...
2. **Configure Remote Backend in Terraform:**
   ...
   terraform {
     backend "s3" {
       ...
       encrypt        = true
       dynamodb_table = "your-dynamodb-table" #state locking
     }
   }
   ```
